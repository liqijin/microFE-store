<template>
  ...
  >
    <div slot="label" class="flex-row ag-cen ml5">
      ...
      /></span>
      <span v-if="status === 0 && field.name === 'ResultInformation'" class="temp-icon ml10" @click="handleResultTemp">
...
export default {
  ...
  computed: {
    ...
    feedBack() {
      if (this.status === 0) {
        ...
        if (this.doubtInfo.active === 'MEDICAL_EVALUATE') {
          ...
          const bool = this.IsAuditBack === '1' || len
          return bool
        }
        return false
      } else {
    ...
    isDisabledFunc() {
      if (this.infoType === 'view') return true
  ...
  methods: {
    ...
    handleFidle(e) {
      if (e === '' || e === null) return
      if (typeof e === 'object' && (e.target.value === '' || e.target.value === null)) return
    ...
    handleMedDraData() {
      ...
      const props = { relate: this.field.relate }
      this.$dialog.invoke({
        title: 'MedDRA字典',
        width: '80vw',
        render: h =>
