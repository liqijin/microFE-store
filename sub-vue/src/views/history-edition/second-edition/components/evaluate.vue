...
import DrugEvaluation from '../table-com/drug-evaluation.vue'
  ...
  methods: {
    ...
    getOptionList() {
      this.formItemList.forEach(async el => {
        if (el.name && el.prop && el.name !== el.prop) {
          const apiServe = {
    ...
    async operateSubDetailServe(val) {
      ...
      for (const key in data) {
        if (key === 'IsUnexpected' && data[key] === '') data[key] = -1
      }
