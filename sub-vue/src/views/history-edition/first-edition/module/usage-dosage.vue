        ...
        <template slot-scope="scope">
          <div class="flex-row ag-cen">
            ...
            <div v-else-if="item.inputType === 'number'">
              <el-input v-model.number="scope.row[item.prop]" v-decimal.zz placeholder="" />
            </div>
            <div v-else-if="item.inputType === 'select'">
              <el-select v-model="scope.row[item.prop]" filterable placeholder="">
    ...
    feedBack() {
      if (this.doubtInfo.pageState === 0) {
        ...
        if (this.doubtInfo.active === 'MEDICAL_EVALUATE') {
          return this.doubtInfo.IsAuditBack === '1' || (this.field.QuestionList && this.field.QuestionList.length)
        }
        return false
      } else {
        return false
